...
def syncTimesheet(page,count):
    ...
    for ts_id, ts in timesheets.items():
        ...
        for cf_id, value in ts.get("customfields", {}).items():
            cf_res = supabase_client.table("timesheet_customfield_values").upsert({
                "timesheet_id": ts["id"],
...
async def syncFiles():
    ...
    for ts_id, fil in files.items():
        fs_data = {
            "id": int(fil["id"]),
            "uploaded_by_user_id": int(fil["uploaded_by_user_id"]) if fil.get("uploaded_by_user_id") else None,
